...
def test_executor_remove_builtin_end_to_end_removes_files(
    config: Config,
    pool: RepositoryPool,
    io: BufferedIO,
    env: MockEnv,
) -> None:
    # End-to-end: with the builtin uninstaller enabled, Executor._remove must
    # actually delete an installed distribution's files from disk. Unlike the
    # wiring tests above, uninstall_distribution is NOT mocked here.
    config.merge({"installer": {"builtin-uninstall": True}})
    ...
    init_py.write_text("# demo\n", encoding="utf-8")

    dist_info = purelib / "demo-1.0.0.dist-info"
    dist_info.mkdir()
    ...
    with (dist_info / "RECORD").open("w", newline="", encoding="utf-8") as fh:
        ...
        writer.writerow(("demo/__init__.py", "", ""))
        writer.writerow(("demo-1.0.0.dist-info/METADATA", "", ""))
        writer.writerow(("demo-1.0.0.dist-info/RECORD", "", ""))

    executor = Executor(env, pool, config, io)

    assert executor._remove(Package("demo", "1.0.0")) == 0
    # No pip subprocess was spawned ...
    assert env.executed == []
    # ... and the distribution's files (and .dist-info) are gone from disk.
    assert not init_py.exists()
    assert not pkg_dir.exists()
    assert not dist_info.exists()


def test_uninstall_builtin_failure_propagates(
    config: Config,
    pool: RepositoryPool,
    io: BufferedIO,
    env: MockEnv,
    ...
    # The builtin uninstaller raises when a file cannot be removed. On a
    # standalone uninstall operation that error must propagate (so the operation
    # is reported as Failed) rather than being swallowed into a success.
    config.merge({"installer": {"builtin-uninstall": True}})
    mocker.patch(
        "poetry.installation.executor.uninstall_distribution",
        side_effect=OSError("cannot remove file"),
    )

    executor = Executor(env, pool, config, io)
    operation = Uninstall(Package("demo", "1.0.0"))

    with pytest.raises(OSError, match="cannot remove file"):
        executor._do_execute_operation(operation)

    # No pip subprocess is involved on the builtin path.
    assert env.executed == []
...
def test_update_continues_when_builtin_uninstall_returns_none(
    config: Config,
    pool: RepositoryPool,
    io: BufferedIO,
    env: MockEnv,
    ...
    # A no-op builtin uninstall must not block installing the new wheel.
    uninstall_mock.assert_called_once_with(env, "demo")
    wheel_install.assert_called_once()
    assert env.executed == []


def test_update_aborts_install_when_builtin_uninstall_fails(
    config: Config,
    pool: RepositoryPool,
    io: BufferedIO,
    env: MockEnv,
    ...
    # The builtin uninstaller raises when a file cannot be removed. The update
    # must be aborted so the new version is not installed over the old one.
    config.merge({"installer": {"builtin-uninstall": True}})
    mocker.patch(
        "poetry.installation.executor.uninstall_distribution",
        side_effect=OSError("cannot remove file"),
    ...
    # The new version must not be installed over the old one.
...
def test_update_install_failure_propagates_with_pip_path(
    config: Config,
    pool: RepositoryPool,
    io: BufferedIO,
    env: MockEnv,
    ...
    # Default config: builtin-uninstall is False, so the pip path is used and the
    # builtin uninstaller is not involved. An install failure must still propagate.
