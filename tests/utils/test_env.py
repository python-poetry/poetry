...
def test_virtualenvs_with_spaces_in_their_path_work_as_expected(
    ...
    venv_path = Path(tmp_path) / "Virtual Env"
...
def test_activate_activates_non_existing_virtualenv_no_envs_file(
    ...
    config.merge({"virtualenvs": {"path": Path(tmp_path)}})
    ...
    m.assert_called_with(
        Path(tmp_path) / f"{venv_name}-py3.7",
    ...
    envs_file = TOMLFile(Path(tmp_path) / "envs.toml")
    assert envs_file.exists()
    envs = envs_file.read()
    assert envs[venv_name]["minor"] == "3.7"
    assert envs[venv_name]["patch"] == "3.7.1"

    assert env.path == Path(tmp_path) / f"{venv_name}-py3.7"
...
def test_activate_fails_when_python_cannot_be_found(
    ...
    os.mkdir(os.path.join(tmp_path, f"{venv_name}-py3.7"))
...
def test_activate_activates_different_virtualenv_with_envs_file(
    ...
    m.assert_called_with(
        Path(tmp_path) / f"{venv_name}-py3.6",
    ...
    assert env.path == Path(tmp_path) / f"{venv_name}-py3.6"
    assert env.base == Path("/prefix")
...
def test_activate_activates_recreates_for_different_patch(
    ...
    remove_venv_m.assert_called_with(Path(tmp_path) / f"{venv_name}-py3.7")
    ...
    assert env.path == Path(tmp_path) / f"{venv_name}-py3.7"
    assert env.base == Path("/prefix")
    assert (Path(tmp_path) / f"{venv_name}-py3.7").exists()
...
def test_activate_does_not_recreate_when_switching_minor(
    ...
    os.mkdir(os.path.join(tmp_path, f"{venv_name}-py3.7"))
    os.mkdir(os.path.join(tmp_path, f"{venv_name}-py3.6"))
    ...
    assert env.path == Path(tmp_path) / f"{venv_name}-py3.6"
    assert env.base == Path("/prefix")
    assert (Path(tmp_path) / f"{venv_name}-py3.6").exists()
...
def test_deactivate_non_activated_but_existing(
    ...
    (Path(tmp_path) / f"{venv_name}-py{python}").mkdir()
    ...
    assert env.path == Path(tmp_path) / f"{venv_name}-py{python}"
...
def test_deactivate_activated(
    ...
    (Path(tmp_path) / f"{venv_name}-py{version.major}.{version.minor}").mkdir()
    (
        Path(tmp_path) / f"{venv_name}-py{other_version.major}.{other_version.minor}"
    ).mkdir()
    ...
    assert env.path == Path(tmp_path) / f"{venv_name}-py{version.major}.{version.minor}"
...
def test_get_prefers_explicitly_activated_virtualenvs_over_env_var(
    ...
    config.merge({"virtualenvs": {"path": Path(tmp_path)}})
    (Path(tmp_path) / f"{venv_name}-py3.7").mkdir()
...
def test_list(
    ...
    (Path(tmp_path) / f"{venv_name}-py3.7").mkdir()
    (Path(tmp_path) / f"{venv_name}-py3.6").mkdir()

    venvs = manager.list()

    assert len(venvs) == 2
    assert venvs[0].path == (Path(tmp_path) / f"{venv_name}-py3.6")
    assert venvs[1].path == (Path(tmp_path) / f"{venv_name}-py3.7")
...
def test_remove_by_python_version(
    ...
    expected_venv_path = Path(tmp_path) / f"{venv_name}-py3.6"
...
def test_raises_if_acting_on_different_project_by_full_path(
    ...
    different_venv_path = Path(tmp_path) / f"{different_venv_name}-py3.6"
...
def test_raises_if_acting_on_different_project_by_name(
    ...
    different_venv_path = Path(tmp_path) / different_venv_name
    different_venv_bin_path = different_venv_path / "bin"
    different_venv_bin_path.mkdir(parents=True)
...
def test_raises_when_passing_old_env_after_dir_rename(
    ...
    venv_path = Path(tmp_path) / f"{venv_name}-py3.6"
    venv_path.mkdir()

    previous_venv_name = f"{previous_venv_name}-py3.6"
    previous_venv_path = Path(tmp_path) / previous_venv_name
...
def test_activate_with_in_project_setting_does_not_fail_if_no_venvs_dir(
    ...
    config.merge(
        {
            "virtualenvs": {
                "path": str(Path(tmp_path) / "virtualenvs"),
                "in-project": True,
            }
        }
    )
    ...
    envs_file = TOMLFile(Path(tmp_path) / "virtualenvs" / "envs.toml")
...
def test_env_finds_the_correct_executables_for_generic_env(
    ...
    venv_path = Path(tmp_path) / "Virtual Env"
    child_venv_path = Path(tmp_path) / "Child Virtual Env"
    manager.build_venv(venv_path, with_pip=True)
...
def test_build_environment_called_build_script_specified(
    ...
    project_env = MockEnv(path=Path(tmp_path) / "project")
    ephemeral_env = MockEnv(path=Path(tmp_path) / "ephemeral")
