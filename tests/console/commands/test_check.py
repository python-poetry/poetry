...
def test_check_valid_legacy(
    ...
    expected = (
        ...
        "Warning: [tool.poetry.dependencies] is set but [project.dependencies] is not "
        "and 'dependencies' is not in [project.dynamic]. You should either migrate "
        "[tool.poetry.depencencies] to [project.dependencies] (if you do not need "
        "Poetry-specific features) or add [project.dependencies] in addition to "
        "[tool.poetry.dependencies] or add 'dependencies' to [project.dynamic].\n"
        "Warning: [tool.poetry.dependencies.python] is set but "
        "[project.requires-python] is not set and 'requires-python' is not in "
        "[project.dynamic].\n"
    )
