...
class TestWrapperCommand:
    ...
    def test_is_valid_version(self):
        ...
        assert cmd._is_valid_version("10.20.30") is True, (
            "Version 10.20.30 should be valid"
        )

        # Invalid versions
        assert cmd._is_valid_version("1.2") is False, "Version 1.2 should be invalid"
        assert cmd._is_valid_version("1.2.3.4") is False, (
            "Version 1.2.3.4 should be invalid"
        )
        assert cmd._is_valid_version("1.2.a") is False, (
            "Version 1.2.a should be invalid"
        )
        assert cmd._is_valid_version("abc") is False, (
            "Non-numeric version should be invalid"
        )
    ...
    def test_generate_script_content(self, mock_datetime):
        ...
        assert f"Poetry version {version}" in script_content, (
            "Script should include version"
        )
        assert "Generated by `poetry wrapper` on 2025-04-12" in script_content, (
            "Script should include date"
        )
        assert (
            'POETRY_HOME="$HOME/.poetry/versions/$POETRY_VERSION"' in script_content
        ), "Script should set POETRY_HOME"
        assert (
            'if ! curl -sSL https://install.python-poetry.org | POETRY_HOME="$POETRY_HOME" python3 - --version "$POETRY_VERSION"; then'
            in script_content
        ), "Script should include installation command"
        assert 'if [ ! -f "$POETRY_HOME/bin/poetry" ]; then' in script_content, (
            "Script should check for poetry executable after installation"
        )
        assert (
            'echo "Error: Poetry executable not found at $POETRY_HOME/bin/poetry after installation."'
            in script_content
        ), "Script should include post-installation error message"
        assert 'exec "$POETRY_HOME/bin/poetry" "$@"' in script_content, (
            "Script should exec poetry"
        )
    ...
    def test_make_executable_on_non_posix(self, tmp_path: Path):
        ...
        with patch("os.name", "nt"):  # Simulate Windows
            ...
            assert (
                "Setting executable permissions is not supported on this platform."
                in output
            ), "Should warn about unsupported platform"
    ...
    def test_handle_with_default_version(self, tmp_path, mock_datetime):
        ...
        assert f"Poetry version {__version__}" in script_file.read_text(), (
            "Script should include version"
        )
    ...
    def test_handle_with_custom_version(self, tmp_path, mock_datetime):
        ...
        with (
            ...
            patch.object(cmd, "option", return_value=custom_version),
        ):
            result = cmd.handle()
        ...
        assert script_file.exists(), "Script file should be created"
        assert f"Poetry version {custom_version}" in script_file.read_text(), (
            "Script should include custom version"
        )
    ...
    def test_handle_with_invalid_version(self, tmp_path, mock_datetime):
        ...
        with (
            patch("pathlib.Path.cwd", return_value=tmp_path),
            patch.object(cmd, "option", return_value=invalid_version),
        ):
            result = cmd.handle()
    ...
    def test_handle_with_existing_files(self, tmp_path, mock_datetime):
        ...
        output = cmd._io.fetch_output()
        assert (
            "Warning: Existing wrapper files detected. They will be overwritten."
            in output
        )
        assert (
            f"Poetry wrapper generated successfully for version {__version__}" in output
        )
        ...
        assert f"Poetry version {__version__}" in script_file.read_text(), (
            "Script file should be overwritten"
        )
