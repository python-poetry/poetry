...
"""
    ...
    assert 1 == command.installer.executor.installations_count
...
"""
    ...
    assert expected == tester.io.fetch_output()
    assert 2 == command.installer.executor.installations_count
...
def test_add_git_constraint(app, repo, installer, tmp_venv):
    command = app.find("add")
    command.set_env(tmp_venv)
    tester = CommandTester(command)
...
def test_add_git_constraint_with_poetry(app, repo, installer, tmp_venv):
    command = app.find("add")
    command.set_env(tmp_venv)
    tester = CommandTester(command)
...
def test_add_git_constraint_with_extras(app, repo, installer, tmp_venv):
    command = app.find("add")
    command.set_env(tmp_venv)
    tester = CommandTester(command)
...
"""
    ...
    assert expected == tester.io.fetch_output()
    assert 4 == command.installer.executor.installations_count
...
def test_add_git_ssh_constraint(app, repo, installer, tmp_venv):
    command = app.find("add")
    command.set_env(tmp_venv)
    tester = CommandTester(command)
...
def test_add_file_constraint_wheel(app, repo, installer, mocker, poetry):
    p = mocker.patch("poetry.utils._compat.Path.cwd")
...
def test_add_url_constraint_wheel(app, repo, installer, mocker):
    download_mock = mocker.patch(
        ...
        return_value=Path(__file__).parent.joinpath(
...
def test_add_no_constraint_old_installer(app, repo, installer, new_installer_disabled):
    command = app.find("add")
    tester = CommandTester(command)
...
def test_add_equal_constraint_old_installer(
    app, repo, installer, new_installer_disabled
):
    command = app.find("add")
    tester = CommandTester(command)
...
def test_add_git_constraint_old_installer(app, repo, installer, new_installer_disabled):
    command = app.find("add")
    tester = CommandTester(command)
...
def test_add_directory_constraint_old_installer(
    app, repo, installer, mocker, new_installer_disabled
):
    p = mocker.patch("poetry.utils._compat.Path.cwd")
    p.return_value = Path(__file__) / ".."
...
def test_add_constraint_with_platform_old_installer(
    app, repo, installer, env, new_installer_disabled
):
    platform = sys.platform
    env._platform = platform
    command = app.find("add")
...
def test_add_constraint_with_source_old_installer(
    app, poetry, installer, new_installer_disabled
):
    repo = LegacyRepository(name="my-index", url="https://my-index.fake")
    repo.add_package(get_package("cachy", "0.2.0"))
    repo._cache.store("matches").put("cachy:0.2.0", [Version.parse("0.2.0")], 5)
...
def test_add_constraint_not_found_with_source_old_installer(
    app, poetry, mocker, new_installer_disabled
):
    repo = LegacyRepository(name="my-index", url="https://my-index.fake")
    mocker.patch.object(repo, "find_packages", return_value=[])
...
def test_add_with_lock_old_installer(app, repo, installer, new_installer_disabled):
    command = app.find("add")
    tester = CommandTester(command)
