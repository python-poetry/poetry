...
class Env(ABC):
    ...
    def scheme_dict(self) -> dict[str, str]:
        ...
        If all candidates in `self.paths` are writable, no modification is made. Otherwise, alternative
        candidates are generated for the paths that are not writable by relocating them below "userbase".
        If any candidate is not writable, the original paths are returned.

        Alternative writable candidates are generated by replacing the discovered prefix with "userbase"
        if available. The original prefix is computed as the common path prefix of the paths that
        are not writable. For example, given `{ "purelib": "/usr/local/lib/python3.13/site-packages",
        "scripts": "/usr/local/bin", "userbase": "/home/user/.local" }` where neither path is writable;
        the candidate "purelib" path would be `/home/user/.local/lib/python3.13/site-packages`.

        Paths that are already writable are never modified. This is relevant for project plugins,
        where "purelib" and "platlib" are redirected to the project's plugin cache
        (see `ProjectPluginCache._install()`) and must not be relocated below "userbase".
        """
        ...
        if (
            ...
            if not read_only_keys:
                # all paths are writable, return early
                return paths

            try:
                base_path = os.path.commonpath([paths[key] for key in read_only_keys])
            except ValueError:
                return paths

            if base_path == os.path.dirname(base_path):
                # The read-only paths only have the filesystem root in common,
                # so there is no meaningful prefix that could be replaced
                # with "userbase". We cannot do much here.
                return paths

            for key in read_only_keys:
                candidate = os.path.normpath(
                    os.path.join(
                        paths["userbase"], os.path.relpath(paths[key], base_path)
                    )
                )
