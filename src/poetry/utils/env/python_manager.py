...
class Python:
    ...
    def _detect_active_python(io: IO) -> Path | None:
        ...
        executable = Python._full_python_path("python3")
    ...
    def get_by_name(cls, python_name: str) -> Python | None:
        _executable = cls._full_python_path(python_name)
        if not _executable:
            return None

        return cls(executable=_executable)

    @classmethod
    def get_preferred_python(cls, config: Config, io: None | IO = None) -> Python:
        io = io or NullIO()
        ...
        if config.get("virtualenvs.prefer-active-python") and (
            ...
            _executable = active_python

        return cls(executable=_executable)

    @classmethod
    def get_compatible_python(cls, poetry: Poetry, io: None | IO = None) -> Python:
        io = io or NullIO()
        ...
        for suffix in sorted(
            ...
            python = cls(executable)

            if supported_python.allows(python.patch_version):
                io.write_error_line(
